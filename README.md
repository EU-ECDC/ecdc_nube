...
### Signals
Analyses are triggered through signals in json format. NUBE constantly checks for signals appearing on `data/signals/` and triggers the appropriate analyses based on type of input data and the expected outputs, which are defined in the "experiment_list" portion of the signal. Here is an example of a signal:
...
Each json should contain the information needed to run one sample, with the sample ID specified as the key as seen above.
